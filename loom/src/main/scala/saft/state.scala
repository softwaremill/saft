...
case class LeaderState(
    ...
    awaitingResponses: Vector[(LogIndex, () => Unit)]
):
  ...
  def addAwaitingResponse(index: LogIndex, respond: () => Unit): LeaderState =
    copy(awaitingResponses = awaitingResponses :+ (index, respond))

  def removeAwaitingResponses(upToIndex: LogIndex): (LeaderState, Vector[() => Unit]) =
